...
The full Windows build and dependency-refresh workflows use pinned UV `0.11.29` to install `cpython-3.13.14-windows-x86_64-none` under the ephemeral runner directory with `--no-registry` and `--no-bin`. The resulting absolute path is used explicitly for project synchronization, payload preparation, and as the controlled reuse fixture. This avoids the CPython Burn same-version all-user/per-user servicing conflict caused by `actions/setup-python`, while the bundled payload remains the checksum- and Authenticode-verified python.org installer.

The E2E harness captures the explicit reusable interpreter before any installer mutation and checks its canonical path, base-interpreter identity, version, architecture, file length, and SHA-256 after both the private-runtime and reused-runtime uninstalls. It never rediscovers or switches the reuse candidate during the lifecycle, and the isolated PEP 514 registration points only to that captured path.

Failed-staging evidence records the active Python and manifest SHA-256 values before and after the injected failure, plus explicit staging cleanup and previous-environment absence. Both private-runtime and reused-runtime uninstall paths assert application-root, discovery, and Start menu removal while preserving bounded technical logs; the reused-runtime path additionally proves that the captured external interpreter is byte-for-byte unchanged.
